...
IFile *ImageFile::__open_ro_remote(const std::string &dir, const std::string &digest,
    ...
    if (not dir.empty()) {
        ...
        LOG_WARN("empty 'dir' set in config.v1.json, skip background download anyway");
    }
    ...
    if ((!dir.empty()) && conf.HasMember("download") && conf.download().enable() == 1) {
...
int ImageFile::open_lower_layer(IFile *&file, ImageConfigNS::LayerConfig &layer, int index) {
    std::string opened;
    file = open_localfile(layer, opened);
    if (file != nullptr) {
        opened = layer.digest();
